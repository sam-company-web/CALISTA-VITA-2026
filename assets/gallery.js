...
var index=0,key='jewellery',timer;
...
function render(){var list=data[key]||[];if(!list.length)return;index=(index+list.length)%list.length;img.src=list[index];img.alt='CALISTA VITA '+names[key]+' image '+(index+1);title.textContent=names[key];count.textContent=(index+1)+' / '+list.length;buttons.forEach(function(b){b.setAttribute('aria-selected',b.getAttribute('data-category')===key?'true':'false');});}
function next(){index++;render();reset();}
function prev(){index--;render();reset();}
function reset(){clearInterval(timer);timer=setInterval(next,9000);}
buttons.forEach(function(b){b.addEventListener('click',function(){key=b.getAttribute('data-category');index=0;render();reset();});});
root.querySelector('[data-next]').addEventListener('click',next);root.querySelector('[data-prev]').addEventListener('click',prev);
var startX=0,startY=0;
img.addEventListener('touchstart',function(e){var t=e.changedTouches[0];startX=t.clientX;startY=t.clientY;},{passive:true});
img.addEventListener('touchend',function(e){var t=e.changedTouches[0],dx=t.clientX-startX,dy=t.clientY-startY;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)){dx<0?next():prev();}},{passive:true});
